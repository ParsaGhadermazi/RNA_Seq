...
getwd()
countsData <- read.table(file = "03_feature/counts.txt", header = FALSE, row.names = 1, skip = 2) # 
...
metadata1 <- read.table(file = "metadata.txt", header = FALSE) # import the data
metadata1


# Organize the metadata file by adding somme column headers
colnames(metadata1) <- c("fasta1", "fasta2", "names1", "names2", "timepoint", "treatment", "rep")
metadata1
...
coldata <- metadata1[,c("timepoint", "treatment", "rep")]
...
dds$treatment <- factor(dds$treatment, levels = c("Acetate","Control"))
dds$timepoint <- factor(dds$timepoint, levels = c("45", "120", "200"))
...
res_Treatment <- results(dds,
                          lfc = 0.5,
                          contrast=c("treatment", "Acetate", "Control"))
...
resLFC_Treatment <- lfcShrink(dds, coef="treatment_Control_vs_Acetate",lfcThreshold=0.5)
...
plotMA(res_Treatment, main="E.coli v. B. subtilus \nunshrunken", 
       ylab = "log fold change (ratio of normalized E.coli / B.subtilis)",
       xlab = "means of normalized counts")

plotMA(resLFC_Treatment, main="E.coli v. B. subtilus \nshrunken", 
       ylab = "log fold change (ratio of normalized E.coli / B.subtilis)",
       xlab = "means of normalized counts")
